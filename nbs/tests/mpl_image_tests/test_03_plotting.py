...
def test_11_inset_plots():
    ...
    try:
        ...
        iris_path = os.path.join(str(parent), "data", "iris.csv")
        # print(f"iris_path={iris_path}")
        iris = pd.read_csv(iris_path)
        print(iris.head())
    except Exception as e:
        print(f"Error while loading the iris dataset. Reason {e}")

    iris_melt = pd.melt(
        iris.reset_index(), id_vars=["species", "index"], var_name="metric"
    )
    ...
    iris_dabest2 = load(
        data=iris, x="species", y="sepal_width", idx=("setosa", "versicolor")
    )

    iris_dabest3 = load(
        data=iris_melt[iris_melt.species == "setosa"],
        x="metric",
        y="value",
    ...
    # Create Figure.
    fig, ax = plt.subplots(
        nrows=2, ncols=2, figsize=(15, 15), gridspec_kw={"wspace": 0.5}
    )
