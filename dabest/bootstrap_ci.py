...
__effect_sizes = es.__effect_sizes
...
def compute_meandiff_jackknife(x0, x1, jackknives, paired=False):
    ...
    """
    import effectsizes as es # drop when collating functions.
...
def compute_acceleration(jack_dist):
    ...
    # Create namedtuple for easy output
    acc = namedtuple('Acceleration',
                     ['mean_diff', 'cohens_d', 'hedges_g'])

    M_acc = __calc_accel(jack_dist.mean_diff)
    d_acc = __calc_accel(jack_dist.cohens_d)
    g_acc = __calc_accel(jack_dist.hedges_g)

    return acc(M_acc, d_acc, g_acc)
...
def compute_mean_diff_bootstraps(x0, x1, paired=False):
    ...
    boot_indexes = list(zip(create_bootstrap_indexes(len(c)),
                            create_bootstrap_indexes(len(t))))
