...
def ProportionalDataFramePlotter(EffectSizeDataFrame, **plot_kwargs):
    ...
    **plot_kwargs:
        color_col=None
        raw_marker_size=0.1, es_marker_size=9,
        swarm_label=None, contrast_label=None,
        swarm_ylim=None, contrast_ylim=None,
        custom_palette=None, swarm_desat=0.5, halfviolin_desat=1,
        halfviolin_alpha=0.8, 
    ...
    yvar        = EffectSizeDataFrame.yvar
    is_paired   = EffectSizeDataFrame.is_paired
    ci          = EffectSizeDataFrame.ci

    def z_value(ci):
        z = st.norm.ppf((100+ci)/200)
        return z
    ...
    if show_pairs is True:
        ...
        rawdata_plot = sns.swarmplot(data=plot_data, x=xvar, y=yvar,
                                     ax=rawdata_axes,
                                     order=all_plot_groups, hue=color_col,
                                     palette=plot_palette_raw, zorder=1, alpha=0,
                                     **swarmplot_kwargs)
    ...
    for xticklab in rawdata_axes.xaxis.get_ticklabels():
        t = xticklab.get_text()
        N = str(counts.loc[t])
        counts_lst.append(counts.loc[t])
    ...
    if float_contrast is True:
        ...
        for jj, axx in enumerate([rawdata_axes, contrast_axes]):
            ...
            if jj == 0:
                ...
                # Test group's proportion error  
                test_err = z_score*np.sqrt(diff*(1-diff)/(counts_lst[1]))

                # Draw Proportion's error lines
                axx.vlines(0, # x-coordinates
                            ref-control_err, ref+control_err, # y-coordinates, start and end.
                            **reflines_kwargs) 
                axx.vlines(effsize_line_start, # x-coordinates
                            diff-test_err, diff+test_err,# y-coordinates, start and end.
                            **reflines_kwargs) 
    ...
    # Set raw axes y-label.
    swarm_label = "Proportion of Success"
    # swarm_label = plot_kwargs['swarm_label']
    # if swarm_label is None and yvar is None:
    #     swarm_label = "Proportion"
    # elif swarm_label is None and yvar is not None:
    #     swarm_label = yvar
        
    # Place contrast axes y-label.
    contrast_label_dict = {'mean_diff'    : "\u0394 Proportion",
                           'median_diff'  : "median difference",
                           'cohens_d'     : "Cohen's d",
                           'hedges_g'     : "Hedges' g",
                           'cliffs_delta' : "Cliff's delta"}
