...
def _create_two_group_jackknife_indexes(x0, x1, repeated_measures):
    """Creates the jackknife bootstrap for 2 groups."""

    if repeated_measures and len(x0) == len(x1):
...
def compute_meandiff_jackknife(x0, x1, repeated_measures, effect_size):
    """
    Given two arrays, returns the jackknife for their effect size.
    """
    from . import effsize as __es

    jackknives = _create_two_group_jackknife_indexes(x0, x1, repeated_measures)
...
def compute_bootstrapped_diff(x0, x1, repeated_measures, effect_size,
                              resamples=5000, random_seed=12345):
    """Bootstraps the effect_size for 2 groups."""
